...
const Nav = () => {
	...
	return (
		<div className="relative bg-[#fff] flex justify-center items-center h-[60px] md:h-[90px] px-[1.5rem] md:px-[3rem] lg:px-[4rem] z-50">
			<div className="w-full flex justify-between items-center">
				...
				<div className="lg:flex hidden justify-between items-center gap-[4rem]">
					{navData.map(({ id, title, href }) => {
						return (
							<Link key={id} href={`${href}`} className={`text-[1rem] text-[#274c5b] `}>
								{title}
							</Link>
						);
					})}
